...
import { buildSlashCommandSystemPrompt, buildSlashCommandUserPrompt } from '../prompts'
...
export class SlashCommandConverterService {
  ...
  private async convertWithTools(
    ...
    const systemPrompt = buildSlashCommandSystemPrompt({
      availableAgents: availableReferences.agents,
      availableSkills: availableReferences.skills,
    })
    const userPrompt = buildSlashCommandUserPrompt({
      content,
      userArguments,
    })
